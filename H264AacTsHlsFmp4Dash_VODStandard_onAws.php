...
const TEST_NAME = "h264-aac-ts-hls-fmp4-dash-vod-standard";
 ...
 */
$video_encoding_profiles = [
    ['height' => 720, 'bitrate' => 2000000, 'profile' => ProfileH264::MAIN(),
        'mode' => StreamMode::STANDARD(), 'preset' => PresetConfiguration::VOD_STANDARD()],
    ['height' => 270, 'bitrate' => 3000000, 'profile' => ProfileH264::MAIN(),
        'mode' => StreamMode::STANDARD(), 'preset' => PresetConfiguration::VOD_STANDARD()]
];
/**
 * @var array<int, array{
...
try {
    ...
    foreach ($video_encoding_profiles as $video_profile) {
        ...
        $bitmovin_api->encoding->encodings->muxings->ts->create($encoding->id, (new TsMuxing())
            ->name("Video Ts Muxing {$video_profile['height']}p")
...
/**
 * Create an HLS manifest using Ts muxing output.
 */
function createHlsManifest(Encoding $encoding, Output $output, string $output_path): HlsManifest
{
    ...
    $ts_muxings = $bitmovin_api->encoding->encodings->muxings->ts->list($encoding->id);
    foreach ($ts_muxings->items as $muxing) {
