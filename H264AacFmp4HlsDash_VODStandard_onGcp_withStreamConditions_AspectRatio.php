...
use BitmovinApiSdk\Models\Condition;
use BitmovinApiSdk\Models\ConditionOperator;
...
use BitmovinApiSdk\Models\ThumbnailUnit;
use BitmovinApiSdk\Models\ThumbnailAspectMode;
use BitmovinApiSdk\Models\VideoAdaptationSet;

/**
 * This sample demonstrates how to use StreamConditions feature to automatically select an appropriate profile
 * based on the input file's aspect ratio. By defining separate profiles for landscape and portrait orientations
 * and applying StreamCondition, the script customizes encoding based on whether the input is in landscape
 * (aspect ratio >= 1) or portrait (aspect ratio < 1).
 */
const TEST_NAME = "h264-aac-fmp4-hls-dash-vod-standard-with-stream-conditions-aspectratio";

const BITMOVIN_API_KEY="{YOUR API KEY}";
const BITMOVIN_TENANT_ORG_ID="{YOUR ORG ID}";

const GCS_INPUT_BUCKET_NAME="{YOUR INPUT GCS BUCKET NAME}";
const GCS_INPUT_ACCESS_KEY="{YOUR INPUT GCS BUCKET ACCESS KEY}";
const GCS_INPUT_SECRET_KEY="{YOUR INPUT GCS BUCKET SECRET KEY}";

const INPUT_PATH = "{YOUR INPUT FILE PATH}";  // landscape or portrait;

const GCS_OUTPUT_BUCKET_NAME="{YOUR OUTPUT GCS BUCKET NAME}";
const GCS_OUTPUT_ACCESS_KEY="{YOUR OUTPUT GCS BUCKET ACCESS KEY}";
const GCS_OUTPUT_SECRET_KEY="{YOUR OUTPUT GCS BUCKET SECRET KEY}";
...
const THUMBNAIL_POSITIONS = [10.0];  // where to extract Thumbnail in seconds

// Define the required video and audio profiles for landscape and portrait
/**
 * @var array<int, array{
 *     height: int,
 *     bitrate: int,
 ...
 * }> $video_encoding_profiles_landscape
 */
$video_encoding_profiles_landscape = [
    // Aspect ratio > 1 (e.g., 16:9 landscape)
    ['height' => 720, 'bitrate' => 2000000, 'profile' => ProfileH264::HIGH(),
        'mode' => StreamMode::STANDARD(), 'preset' => PresetConfiguration::VOD_STANDARD()],
    ['height' => 270, 'bitrate' => 300000, 'profile' => ProfileH264::HIGH(),
        'mode' => StreamMode::STANDARD(), 'preset' => PresetConfiguration::VOD_STANDARD()],
];
 ...
 *     mode: StreamMode,
 *     preset: PresetConfiguration
 * }> $video_encoding_profiles_portrait
 */
$video_encoding_profiles_portrait = [
    // Aspect ratio < 1 (e.g., 9:16 portrait)
    ['height' => 720, 'bitrate' => 1500000, 'profile' => ProfileH264::HIGH(),
        'mode' => StreamMode::STANDARD(), 'preset' => PresetConfiguration::VOD_STANDARD()],
    ['height' => 270, 'bitrate' => 300000, 'profile' => ProfileH264::HIGH(),
        'mode' => StreamMode::STANDARD(), 'preset' => PresetConfiguration::VOD_STANDARD()],
];
...
try {
    ...
    $input = $bitmovin_api->encoding->inputs->gcs->create((new GcsInput)
        ->bucketName(GCS_INPUT_BUCKET_NAME)
        ->accessKey(GCS_INPUT_ACCESS_KEY)
        ->secretKey(GCS_INPUT_SECRET_KEY));
    $output = $bitmovin_api->encoding->outputs->gcs->create((new GcsOutput)
        ->bucketName(GCS_OUTPUT_BUCKET_NAME)
        ->accessKey(GCS_OUTPUT_ACCESS_KEY)
        ->secretKey(GCS_OUTPUT_SECRET_KEY));

    // Create an Encoding instance
    $encoding = $bitmovin_api->encoding->encodings->create((new Encoding)
        ->name(TEST_NAME)
        ->cloudRegion(CloudRegion::GOOGLE_ASIA_NORTHEAST_1())
    ...
    foreach ($video_encoding_profiles_landscape as $video_profile) {
        ...
        if ($video_profile['height'] === 720) {
            $thumbnail = $bitmovin_api->encoding->encodings->streams->thumbnails->create($encoding->id, $video_stream->id, (new Thumbnail)
                ...
                ->outputs([(new EncodingOutput)
                    ->outputId($output->id)
                    ->outputPath(OUTPUT_BASE_PATH . 'thumbnail/')
                    ->acl([(new AclEntry)
    ...
    // For each video profile, create codec config, stream and muxing
    foreach ($video_encoding_profiles_portrait as $video_profile) {
        $h264_codec = $bitmovin_api->encoding->configurations->video->h264->create((new H264VideoConfiguration)
            ->name("Video Codec {$video_profile['height']}p")
            ->height($video_profile['height'])
            ->bitrate($video_profile['bitrate'])
