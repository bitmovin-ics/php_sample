...
use BitmovinApiSdk\Models\PerTitleFixedResolutionAndBitrateConfiguration;
...
const TEST_NAME = "pte-fixed-crf24-h264-aac-fmp4-hls-dash-vod-standard";
...
// Define the PerTitle configuration settings
// The max bitrate that the PerTitle algorithm can generate
const PER_TITLE_MAX_BITRATE = 2000000;

// The min bitrate that the PerTitle algorithm can generate in the ladder
const PER_TITLE_MIN_BITRATE = 300000;

// A parameter that controls quality. Related to CRF encoding in PerTitle Analytics,
// where a higher value results in lower bitrate, and a lower value results in higher bitrate.
const PER_TITLE_TARGET_QUALITY_CRF = 24;
 ...
 */
$video_encoding_profiles = [
    ['height' => 720, 'bitrate' => null, 'profile' => ProfileH264::HIGH(),
        'mode' => StreamMode::PER_TITLE_TEMPLATE(), 'preset' => PresetConfiguration::VOD_STANDARD()],
    ['height' => 270, 'bitrate' => null, 'profile' => ProfileH264::HIGH(),
        'mode' => StreamMode::PER_TITLE_TEMPLATE_FIXED_RESOLUTION(), 'preset' => PresetConfiguration::VOD_STANDARD()]
];
/**
 * @var array<int, array{
 *     bitrate: int,
...
try {
    ...
    foreach ($video_encoding_profiles as $video_profile) {
        $h264_config = (new H264VideoConfiguration)
        ...
        if ($video_profile['bitrate'] !== null) {
            $h264_config->bitrate($video_profile['bitrate']);
        }
        $h264_codec = $bitmovin_api->encoding->configurations->video->h264->create($h264_config);
        ...
        $video_muxing_output = (new EncodingOutput)
            ->outputId($output->id)
            ->outputPath(OUTPUT_BASE_PATH . 'video/{height}_{bitrate}_{uuid}')
            ->acl([(new AclEntry)
    ...
    executeEncoding($encoding, (new StartEncodingRequest)
        ->perTitle((new PerTitle)
            ->h264Configuration((new H264PerTitleConfiguration)
                ->maxBitrate(PER_TITLE_MAX_BITRATE)
                ->minBitrate(PER_TITLE_MIN_BITRATE)
                ->targetQualityCrf(PER_TITLE_TARGET_QUALITY_CRF)
                ->minBitrateStepSize(ceil((sqrt(PER_TITLE_MAX_BITRATE / PER_TITLE_MIN_BITRATE)) * 100) / 100 + 0.01)
                ->maxBitrateStepSize(ceil((PER_TITLE_MAX_BITRATE / PER_TITLE_MIN_BITRATE) * 100) / 100 + 0.01)
                ->codecBufsizeFactor(2.0)
                ->codecMaxBitrateFactor(1.2)
                ->codecMinBitrateFactor(0.8)
            )
        )
    );
    ...
    executeHlsManifestGeneration($hls_manifest, (new StartManifestRequest)
        ->manifestGenerator(ManifestGenerator::V2()));

    // Start the DASH Manifest creation
    executeDashManifestGeneration($dash_manifest, (new StartManifestRequest)
        ->manifestGenerator(ManifestGenerator::V2()));
 ...
 * @throws Exception
 */
function executeHlsManifestGeneration(HlsManifest $hls_manifest, StartManifestRequest $start_manifest_request)
{
    global $bitmovin_api;
 ...
 * state
 */
function executeDashManifestGeneration(DashManifest $dash_manifest, StartManifestRequest $start_manifest_request)
{
    global $bitmovin_api;

    $bitmovin_api->encoding->manifests->dash->start($dash_manifest->id, $start_manifest_request);

    do {
        sleep(5);
        $task = $bitmovin_api->encoding->manifests->dash->status($dash_manifest->id);
        echo 'Manifest status is ' . $task->status . ' (progress: ' . $task->progress . ' %)' . PHP_EOL;
    } while ($task->status != Status::FINISHED() && $task->status != Status::ERROR());
